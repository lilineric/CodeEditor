...
(function ($) {
    $.parser = {
        ...
        }, plugins: ["lookup", "uploadImg", "textarea", "textbox", "draggable", "droppable", "resizable", "pagination", "tooltip", "linkbutton", "menu", "menubutton", "splitbutton", "progressbar", "tree", "combobox", "combotree", "combogrid", "numberbox", "searchbox", "numberspinner", "timespinner", "calendar", "datebox", "datetimebox", "slider", "layout", "panel", "datagrid", "propertygrid", "treegrid", "tabs", "accordion", "window", "dialog"], parse: function (_2) {
            var aa = [];
...
(function ($) {
    ...
    $.fn.pagination.defaults = {
        total: 1, pageSize: 10, pageNumber: 1, pageList: [10, 15, 20, 30, 50], loading: false, buttons: null, showPageList: true, showRefresh: true, onSelectPage: function (_9c, _9d) {
...
(function ($) {
    ...
    function _1d7(_1d8) {
        ...
        if (opts.title && !opts.noheader) {
            ...
            // 添加刷新功能
            if (opts.refresh) {
                $("<a class=\"panel-tool-refresh\" href=\"javascript:void(0)\"></a>").appendTo(tool).bind("click", function () {
                    $(_1d8).html($(_1d8).html())
                    return false;
                });
            }
    ...
    function _233(_234) {
        ...
        _235.window.draggable({
            handle: ">div.panel-header>div.panel-title", disabled: _235.options.draggable == false, onStartDrag: function (e) {
                // 阻止模态框在移动时改变z-index modified by hongfei
                //if (_235.mask) {
...
(function ($) {
    ...
    function _2e1(_2fb, _2fc) {
        ...
        // modified by hongfei
        if (_2fe.find('iframe').length) {
            if (_2fe.find('iframe').attr('src') != 'about:blank') {
                if ($(_2fe.find('iframe')[0].contentDocument).find('.datagrid-view').css('height') === '0px') {
                    var iframe = _2fe.find('iframe').clone();
                    _2fe.html(iframe);
                    $(_2fe.find('iframe')[0].contentDocument).find('.datagrid-view').height(opts.height);
                };
            };
        };
        // end
...
(function ($) {
    function _3ef(_3f0, _3f1) {
        ...
        if (form.find("input[name=__RequestVerificationToken]").length == 0) {
            form.prepend($("#requestVerificationToken input").clone());
        }
        if (form.find("input[name=__RequestVerificationToken]").val().trim() == "") {
            form.find("input[name=__RequestVerificationToken]").val($("#requestVerificationToken input").val());
        }
        // 将form提交模拟为ajax请求
        ...
        // added by hongfei
        // 支持表单文件上传
        if (form.find(".easyui-file").length > 0) {
            form.attr("encType", "multipart/form-data");
        }
        // 支持上传控件
        if (form.find(".easyui-uploadImg").length > 0 &&
            form.find("input[name=SimulateUpload]").length == 0) {
            form.prepend("<input type='hidden' name='SimulateUpload' value='true' />");
        }
        // end
        var _3f5 = $();
        try {
            ...
            // modified by hongfei - 提交查询条件时，不用做form的初始化
            // form[0].submit();
            if (form.data("dontInit") == true) {
                form[0].submit();
            } else {
                form.form("initBeforeSubmit");
                form[0].submit();
                form.form("initRollback");
            }
            // end
        }
        finally {
    ...
    function load(_3f7, data) {
        ...
        function _3f9(data) {
            ...
            for (var name in data) {
                ...
                if (!rr.length) {
                    ...
                    if (f.length) {
                        ...
                        // added by liuzhimin - modified by hongfei
                        if (form.find(".easyui-uploadImg input[name=\"" + name + "\"]").length) {
                            form.find('.easyui-uploadImg').uploadImg("setValue", val);
                        }
                        // end
                    }
                }
            ...
            // 保存form的原始数据，以便submitModified方法只提交修改了的数据 - by hongfei
            form.form("initBeforeSubmit");
            form.data("post_data", form.serializeArray());
            form.form("initRollback");
            // end
                ...
                for (var i = 0; i < cc.length; i++) {
                    ...
                    if (c.hasClass(type + "-f")) {
                        if (c[type]("options").multiple) {
                            ...
                            // modified by hongfei
                            // c[type]("setValue", val);
                            if (type == "datebox" || type == "datetimebox") {
                                if (val == "1900-01-01 00:00:00") {
                                    c[type]("setValue", "");
                                    continue;
                                }
                                if (type == "datebox" && val.length > 10) {
                                    var dateval = val.substr(0, 10);
                                    c[type]("setValue", dateval);
                                } else {
                                    c[type]("setValue", val);
                                }
                            } else {
    ...
    $.fn.form = function (_406, _407) {
        ...
        return this.each(function () {
            ...
            // added by hongfei
            $.data(this, 'form').options = $.extend({}, $.data(this, 'form').options, _406);
            // end
            _401(this);
        });
    };
    $.fn.form.methods = {
        submit: function (jq, _408) {
            return jq.each(function () {
                ...
                _3ef(this, $.extend({}, $.data(this, 'form').options, _408 || {}));
                // end
            });
...
(function ($) {
    function init(_40b) {
        ...
        if ($(_40b).attr("alwaysSubmit") == "true") {
            $(_40b).removeAttr("alwaysSubmit");
            var v = $("<input type=\"hidden\" alwaysSubmit=\"true\">").insertAfter(_40b);
        } else {
            v = $("<input type=\"hidden\">").insertAfter(_40b);
        }
        // end
...
(function ($) {
    ...
    function show(_441) {
        ...
        /*added by liuzhimin*/
        if (opts.minDate) {
            if (new Date(opts.minDate) != 'Invalid Date') {
                t.find('td').each(function () {
                    var _this = $(this);
                    var abbr = _this.attr('abbr').replace(/\,/g, '/');
                    if (new Date(abbr + ' 23:59:59') < new Date(opts.minDate)) {
                        _this.addClass("disable-date");
                    }
                })
            } else {
                if ($(opts.minDate).datebox('getValue') != '') {
                    var datebox = $(opts.minDate).datebox('getValue');
                    //var imMinDate = datebox.split('-')[2] + '/' + datebox.split('/')[0] + '/' + datebox.split('/')[1];
                    t.find('td').each(function () {
                        var _this = $(this);
                        var abbr = _this.attr('abbr').replace(/\,/g, '/');
                        if (new Date(abbr + ' 23:59:59') < new Date(datebox)) {
                            _this.addClass("disable-date");
                        }
                    })
                }
        ...
        if (opts.maxDate) {
            if (new Date(opts.maxDate) != 'Invalid Date') {
                t.find('td').each(function () {
                    var _this = $(this);
                    var abbr = _this.attr('abbr').replace(/\,/g, '/');
                    if (new Date(abbr + ' 00:00:00') > new Date(opts.maxDate)) {
                        _this.addClass("disable-date");
                    }
                })
            } else {
                if ($(opts.maxDate).datebox('getValue') != '') {
                    var datebox = $(opts.maxDate).datebox('getValue');
                    t.find('td').each(function () {
                        var _this = $(this);
                        var abbr = _this.attr('abbr').replace(/\,/g, '/');
                        if (new Date(abbr + ' 00:00:00') > new Date(datebox)) {
                            _this.addClass("disable-date");
                        }
                    })
                }
        ...
        if (opts.minTarget) {
            if (opts.minTarget.datebox('getValue') != '') {
                var datebox = '';
                if (opts.widget === 'datebox') {
                    datebox = opts.minTarget.datebox('getValue').replace(/\-/g, '/');
                } else {
                    datebox = opts.minTarget.datetimebox('getValue').replace(/\-/g, '/');
                };
                var _this = '';
                t.find('td').each(function () {
                    _this = $(this);
        ...
        t.find("td").hover(function () {
            ...
            if (!$(this).hasClass('disable-date')) {
                ...
                if (opts.minTarget) {
                    //if (opts.widget == 'datebox') {
                    //    opts.minTarget.datebox({
                    //        maxDate: opts.current
                    //    });
                    //}
                };
                if (opts.maxTarget) {
                    if (opts.widget == 'datebox') {
                        opts.maxTarget.datebox({
                            minDate: opts.current
                        });
                    }
                };
            };
    ...
    $.fn.calendar.defaults = {
        // modified by liuzhimin
        minDate: null, maxDate: null, minTarget: null, maxTarget: null, width: 180, height: 180, fit: false, border: true, firstDay: 0, weeks: ["S", "M", "T", "W", "T", "F", "S"], months: ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"], year: new Date().getFullYear(), month: new Date().getMonth() + 1, current: new Date(), onSelect: function (date) { }
        // end
    };
})(jQuery);
(function ($) {
...
(function ($) {
    ...
    function _51b(_51c, data) {
        ...
        var dc = _51d.dc;
        /*added by liuzhimin*/
        $(_51c).datagrid('getPanel').find('.datagrid-header-check>input').removeAttr('checked');
        /*end*/
    ...
    $.fn.datagrid.defaults = $.extend({}, $.fn.panel.defaults, {
        // modified by hongfei
        frozenColumns: undefined, columns: undefined, fitColumns: false, resizeHandle: "right", autoRowHeight: true, toolbar: null, striped: false, method: "post", nowrap: true, idField: null, url: null, data: null, loadMsg: "Processing, please wait ...", rownumbers: false, singleSelect: false, selectOnCheck: true, checkOnSelect: true, pagination: false, pagePosition: "bottom", pageNumber: 1, pageSize: 10, pageList: [6, 10, 20, 30, 40, 50], queryParams: {}, sortName: null, sortOrder: "asc", remoteSort: true, showHeader: true, showFooter: false, scrollbarSize: 18, rowStyler: function (_666, _667) {
...
(function ($) {
    function _786(_787, _788) {
        ...
        if (opts.panelHeight === 'auto') {  // 当高度设置是自动时
            if (opts.maxHeight) {           // 判断有没有设置最大高度
                _78a.panel("resize", { width: (opts.panelWidth ? opts.panelWidth : _789.outerWidth()), height: "auto" });
                _78a.css("maxHeight", opts.maxHeight);
            } else {
                _78a.panel("resize", { width: (opts.panelWidth ? opts.panelWidth : _789.outerWidth()), height: opts.panelHeight });
            }
        } else {
    ...
    function _7c3(_7c4, _7c5) {
        ...
        for (var i = 0; i < _7c5.length; i++) {
            // 为下拉菜单增加alwaysSubmit  modified by hongfei 
            // var _7c8 = $("<input type=\"hidden\" class=\"combo-value\">").appendTo(_7c7);
            var _7c8;
            if (opts.multiple || ($(_7c4).attr("alwaysSubmit") === "true" || $(_7c4).attr("alwaysSubmit") === "false")) {
                $(_7c4).attr("alwaysSubmit", "false");
                _7c8 = $("<input type=\"hidden\" class=\"combo-value\" alwaysSubmit=\"true\" />").appendTo(_7c7);
            } else {
                _7c8 = $("<input type=\"hidden\" class=\"combo-value\" />").appendTo(_7c7);
            }
            // end
            if (name) {
                _7c8.attr("name", name);
    ...
    $.fn.combo.defaults = $.extend({}, $.fn.validatebox.defaults, {
        // modified by hongfei
        maxHeight: null, width: "auto", height: 22, panelWidth: null, panelHeight: 200, multiple: false, selectOnNavigation: true, separator: ",", editable: true, disabled: false, readonly: false, hasDownArrow: true, value: "", delay: 200, deltaX: 19, keyHandler: {
            up: function () {
            }, down: function () {
            }, enter: function () {
            }, query: function (q) {
...
(function ($) {
    ...
    function _7e7(_7ee, _7ef, _7f0, afterLoad) {
        ...
        for (var i = 0; i < _7ef.length; i++) {
            ...
            // modified by hongfei
            // - 给单选下拉菜单设置值时，如果值不在下拉列表中，则自动选择第一项“-请选择-”
            // - 给多选下拉菜单设置值时，如果值不再下拉列表中，则不选中任何项
            var find = false;
            ...
            if (!find && opts.multiple == false && data.length > 0) {
                v = data[0][opts.valueField];
                s = data[0][opts.textField];
            }
            if (afterLoad && !find && opts.multiple == true) {
                continue;
            }
            // end
    ...
    $.fn.combobox.parseOptions = function (_80f) {
        ...
        var result = $.extend({}, $.fn.combo.parseOptions(_80f), $.parser.parseOptions(_80f, ["valueField", "textField", "mode", "method", "url"]));
        if (!result.maxHeight) { result.maxHeight = 200; }
        return result;
        // end
    };
...
(function ($) {
    function _852(_853) {
        ...
        function _855(op) {
            ...
            _854.calendar.calendar({
                // modified by hongfei
                minDate: op.minDate, widget: op.widget, minTarget: op.minTarget, maxTarget: op.maxTarget, maxDate: op.maxDate, fit: true, border: false, onSelect: function (date) {
            ...
            // modified by hongfei
            if (!$(_856).find('.datebox-button').length) {
    ...
    function _85b(_85f, _860) {
        ...
        // added by hongfei
        setDisableDate(_861, _860);
        // end
    };
    /*added by liuzhimin*/
    function setDisableDate(_80f, _80e) {
        if (_80e) {
            if (_80f.options.minTarget) {
                var time = _80f.options.minTarget.datebox('getValue').replace(/\-/g, '/');
                var _this = '';
                _80f.calendar.find('td').each(function () {
                    _this = $(this);
                    var abbr = _this.attr('abbr').replace(/\,/g, '/');
                    if (new Date(abbr + ' 23:59:59') < new Date(time)) {
                        _this.addClass("disable-date");
                    };
                });
            };
    ...
    $.fn.datebox.defaults = $.extend({}, $.fn.combo.defaults, {
        // modified by hongfei
        minDate: null, maxDate: null, minTarget: null, maxTarget: null, widget: 'datebox', panelWidth: 180, panelHeight: "auto", keyHandler: {
            up: function () {
            }, down: function () {
            }, enter: function () {
                _85c(this);
            ...
            //}
            // modified by hongfei
            if (s == "1900-01-01 00:00:00") return new Date();
...
(function ($) {
    ...
    function _874(_875) {
        ...
        /*added bu liuzhimin*/
        var abbr = $(this).parent().siblings('.datebox-calendar-inner').find('.calendar-selected').attr('abbr').split(",");
        opts.current = new Date(abbr[0], parseInt(abbr[1]) - 1, abbr[2]);
        if (opts.minTarget) {
            if (opts.widget == 'datetimebox') {
                var curTime = $(_875).datetimebox("getValue").replace(/-/g, '/');
                var minTime = opts.minTarget.datetimebox("getValue").replace(/-/g, '/');
                if (minTime != '1900/01/01 00:00:00') {
                    if (new Date(curTime) < new Date(minTime)) {
                        $.alert("结束时间不允许小于开始时间");
                        (function () { $(_875).datetimebox("setValue", ""); }).delay(1);
                    }
                }
                //opts.minTarget.datetimebox({
                //    maxDate: opts.current
                //});
            }
        };
    ...
    // modified by hongfei
    $.fn.datetimebox.defaults = $.extend({}, $.fn.datebox.defaults, { widget: 'datetimebox' }, {
